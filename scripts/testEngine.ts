...
import { evaluateSource, resolveRoot, resolveCells, resolveInputs, findSource } from "../src/configEngine.ts";
  ...
  return {
    data: raw.nodes
      .map((node) => {
        ...
        return n.type === "data" && Array.isArray(n.data) ? unflatten(n.data) : null;
      })
      .filter((x) => x !== null),
  };
}

const logPayload = unflattenPayload(JSON.parse(read("planning/samples/log_data.json")));
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const enc = resolveRoot(logPayload, logSource) as any;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const allPlayers: any[] = enc.entityList.filter((e: any) => e.entityType === "PLAYER");
const partyInfo = enc.encounterDamageStats.misc.partyInfo as Record<string, string[]>;

console.log("=== LOG PHASE ===");
console.log("encounter root resolved:", !!enc, "| boss:", enc.currentBossName);
console.log("parties:", Object.entries(partyInfo).map(([k, v]) => `${k}:[${v.join(", ")}]`).join("  "));
...
for (const [num, names] of Object.entries(partyInfo)) {
  ...
  const res = evaluateSource(logSource, logPayload, { players, member }, resolvedInputs);
...
const snapResults = evaluateSource(snapSource, snapPayload, undefined, resolvedInputs);
...
try {
  ...
  const liveResults = evaluateSource(snapSource, livePayload, undefined, resolvedInputs);
  ...
  console.log(
    ...
    `\n  gem errors: ${Object.values(liveResults).filter(r => r.error).map(r => r.error).join("; ") || "none"}`
  );
} catch {
...
for (const file of ["mirsee", "esthie", "edward", "sos69"]) {
  const res = evaluateSource(loadoutSource, loadHydratedSnapshot(`planning/samples/${file}_loadout.js`));
  const r = res.skinBonusFromLoadout;
  console.log(`  ${file.padEnd(8)} skinBonusFromLoadout = ${r.error ? "ERROR " + r.error : r.value}`);
}
...
for (const w of writes) {
  const fmt = w.format ? `  [${w.format.type}${w.format.pattern ? " " + w.format.pattern : ""}]` : "";
  console.log(`  ${w.cell.padEnd(6)} <- ${w.value}${fmt}`);
}
console.log(`\n${writes.length} writes, ${skipped.length} skipped, ${snapErrors} snapshot field errors`);
if (skipped.length) {
  console.log("skipped:");
  for (const s of skipped) console.log(`  ${s.cell.padEnd(6)} ${s.field} (${s.reason})`);
}
