...
check(
  "range 1.46 in >=1.45 <=1.46",
  satisfiesVersion("1.46.0", ">=1.45.0 <=1.46.0"),
  true,
);
check(
  "range 1.44 in >=1.45 <=1.46",
  satisfiesVersion("1.44.0", ">=1.45.0 <=1.46.0"),
  false,
);
...
const payloadWithVersion = (v: string) => ({
  data: [
    {
      encounterInfo: {
        encounter: { encounterDamageStats: { misc: { version: v } } },
      },
    },
  ],
});
...
{
  const sel = selectSourceForPayload(
    [mkLog("<=1.46.0")],
    payloadWithVersion("1.45.0"),
  );
...
check(
  "hash no-prefix -> undefined",
  versionFromLoadoutHash("abc123") ?? null,
  null,
);
...
// Snapshot: version supplied out-of-band (from the loadoutHash) via override.
{
  ...
  const match = selectSourceForPayload(
    [snap],
    payload,
    versionFromLoadoutHash("v3/abc"),
  );
  check("snapshot v3: actual", match.actual, "v3");
  check("snapshot v3: no warning", match.warning ?? null, null);

  const drift = selectSourceForPayload(
    [snap],
    payload,
    versionFromLoadoutHash("v4/abc"),
  );
